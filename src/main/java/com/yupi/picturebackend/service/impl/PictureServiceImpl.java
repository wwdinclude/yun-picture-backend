...
import com.yupi.picturebackend.api.aliyunai.AliYunAiApi;
...
import com.yupi.picturebackend.manager.upload.FilePictureUpload;
import com.yupi.picturebackend.manager.upload.PictureUploadTemplate;
import com.yupi.picturebackend.manager.upload.UrlPictureUpload;
...
import com.yupi.picturebackend.model.enums.PictureReviewStatusEnum;
...
import com.yupi.picturebackend.utils.ColorSimilarUtils;
...
//    private FileManager fileManager;
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null) {
            ...
            if (spaceId == null) {
                ...
                if (!ObjUtil.equal(oldPicture.getSpaceId(), spaceId)) {
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAMS_ERROR, "最多 30 张图片");
        ...
        // 最多只收集 100 张有效图片，超过业务需要的 30 张
        final int MAX_VALID_SIZE = 100;
        List<Element> validImgElements = new ArrayList<>();

        for (Element imgElement : imgElementList) {
            // 如果已收集足够，直接跳出循环
            if (validImgElements.size() >= MAX_VALID_SIZE) {
                break;
            }

            String fileUrl = imgElement.attr("src");
            if (StrUtil.isNotBlank(fileUrl)) {
                validImgElements.add(imgElement);
            }
        }

        int validTotal = validImgElements.size();
        if (validTotal == 0) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "未找到有效图片");
        }

        int startIndex = 0;
        if (validTotal > count) {
            startIndex = RandomUtil.randomInt(0, validTotal - count + 1);
        }

        int uploadCount = 0;
        for (int i = startIndex; i < validTotal && uploadCount < count; i++) {
            Element imgElement = validImgElements.get(i);
            String fileUrl = imgElement.attr("src");
    ...
    public void checkPictureAuth(User loginUser, Picture picture) {
        ...
        if (spaceId == null) {
            // 公共图库，仅本人或管理员可操作
            if (!ObjUtil.equal(loginUserId, picture.getUserId()) && !userService.isAdmin(loginUser)) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
            }
        } else {
            // 私有图库，仅空间管理员可操作
            if (!ObjUtil.equal(loginUserId, picture.getUserId())) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
            }
        }
    }
    ...
    public List<PictureVO> searchPictureByColor(Long spaceId, String picColor, User loginUser) {
        ...
        // 4. 按颜色相似度排序
        List<Picture> sortedPicturesList = pictureList.stream()
        ...
        // 5. 返回结果
        return sortedPicturesList.stream()
